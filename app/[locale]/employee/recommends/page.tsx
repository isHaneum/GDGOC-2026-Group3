...
import {
  type EmployeeRecommendationsAiEvaluation,
...
function nextStepLabelKey(step: DeveloperToCompanyFitResult["recommendedNextStep"]) {
  switch (step) {
    case "apply_now":
      return "nextStepApplyNow";
    case "casual_interview":
      return "nextStepCasualInterview";
    case "trial_project":
      return "nextStepTrialProject";
    case "bridge_labs_activity":
      return "nextStepBridgeLabs";
    case "research_company":
      return "nextStepResearchCompany";
    case "rewrite_motivation":
    default:
      return "nextStepRewriteMotivation";
  }
}

function formatAiEvaluationMessage(
  evaluation: EmployeeRecommendationsAiEvaluation,
  t: ReturnType<typeof useTranslations>
) {
  if (evaluation.evaluatedCompanyCount === 0) {
    return evaluation.geminiConfigured ? t("aiEvaluationReady") : t("aiEvaluationNoGemini");
  }

  if (evaluation.geminiUsedCount > 0) {
    return t("aiEvaluationMixed", {
      total: evaluation.evaluatedCompanyCount,
      gemini: evaluation.geminiUsedCount,
      fallback: evaluation.fallbackCount
    });
  }

  return evaluation.geminiConfigured
    ? t("aiEvaluationAllFallback")
    : t("aiEvaluationFallbackOnly", { total: evaluation.evaluatedCompanyCount });
}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="rounded-full bg-white px-2.5 py-1 text-caption font-bold text-gray-500 ring-1 ring-gray-100">
              {t(nextStepLabelKey(recommendation.recommendedNextStep))}
            </span>
          </div>
            ...
            <div>
              ...
              {payload?.aiEvaluation ? (
                <p className="mt-3 max-w-3xl rounded-xl bg-bridge-paper px-3 py-2 text-caption font-bold text-gray-600">
                  {formatAiEvaluationMessage(payload.aiEvaluation, t)}
                </p>
              ) : null}
              {payload?.developer ? (
    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <span className="rounded-full bg-bridge-primary/15 px-3 py-1 text-caption font-black text-bridge-teal">
          {t("nextAction")}: {t(nextStepLabelKey(recommendation.recommendedNextStep))}
        </span>
      </div>
      ...
      <div className="mt-5 grid gap-4">
        ...
        <InfoPanel title={t("aiEvaluationExplanation")}>
          <p>{recommendation.explanation || t("aiExplanationFallback")}</p>
        </InfoPanel>
