...
export default function PostDetailPage() {
  ...
  async function handleComment(e: React.FormEvent) {
    ...
    try {
      ...
      setComments((prev) => [
        ...prev,
        { ...newComment, post_id: Number(id), author_id: 0, updated_at: newComment.created_at, author: { id: 0, role: 'developer', market: 'KR', developer_profiles: [] } }
      ])
      setCommentText('')
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-bridge-paper flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/community" className="text-sm text-bridge-primary font-bold hover:underline">← 커뮤니티로</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-bridge-paper">
      <div className="container mx-auto max-w-2xl px-4 py-10 space-y-6">
        ...
        <Link href="/community" className="text-sm font-bold text-gray-400 hover:text-bridge-primary transition-colors">
          ← 커뮤니티로
        </Link>
